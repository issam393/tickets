...
async function listRooms(req, res) {
    try {
        const rooms = await roomService.listRoomsForRole(req.user.role);
...
async function getRoomByTicket(req, res) {
    try {
        ...
        roomService.assertRoomAccess(room, req.user.role);
...
async function getRoomHistory(req, res) {
    try {
        const history = await roomService.getRoomHistory(req.params.roomId, req.user.role);
